...
ORAR_URL = r"https://profs.info.uaic.ro/~orar/orar_discipline.html"
...
def generate_orare():
    ...
    data = request.text
    site_title = re.findall(r'<td>\s*<a href="([^"]+)">([^<]+)</a>', data)
    # print(site_title)
...
def get_orare(site, title):
    ...
    print(final_dict)
    with open(r'./orare_discipline_ex/{}'.format(final_dict['title']), 'w') as f:
