...
ORAR_URL = r"https://profs.info.uaic.ro/~orar/orar_resurse.html"
...
def get_orare(site, title):
    ...
    for item in all_days:
        ...
        for i in range(1, len(item)):
            ...
            subject_dict['studenti'] = re.sub('\s+', ' ', item[i][5]).strip()
            # subject_dict['frecventa'] = item[i][6]
            subject_dict['pachet'] = item[i][6]

            day_dict['materie {}'.format(i)] = subject_dict
        final_dict['examene {}'.format(item[0][0])] = day_dict
        # print(day_dict)

    print(final_dict)
    with open(r'./orare_sali_ex/{}'.format(final_dict['title']), 'w') as f:
